...
/** Choix de la zone au dépôt d'un widget : la box, puis la zone. */
class ThermostatWidgetConfigActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (catalogue.isEmpty()) {
            findViewById<TextView>(R.id.config_title)
                .setText(R.string.widget_thermo_config_title)
            ...
            boxes.map { box ->
                ...
                Row(
                    ...
                    resources.getQuantityString(
                        R.plurals.widget_thermo_count, count, count
                    )
                )
            }
        ) { position -> showZones(boxes[position]) }
    }

    private fun showZones(box: String) {
        selectedBox = box
        val zones = catalogue.filter { it.box == box }
        findViewById<TextView>(R.id.config_title).text =
            getString(R.string.widget_thermo_config_zone, box)
        bind(zones.map { Row(it.name, "") }) { position ->
            attach(zones[position].key)
        }
    }
    ...
    private fun attach(key: String) {
        HomeWidgetPlugin.getData(this).edit()
            .putString(ThermostatWidgetProvider.bindingKeyFor(appWidgetId), key)
            .apply()
        AppWidgetManager.getInstance(this).updateAppWidget(
            appWidgetId,
            ThermostatWidgetProvider.build(
    ...
    private data class Entry(val key: String, val box: String, val name: String)

    private fun readCatalogue(): List<Entry> {
        val raw = HomeWidgetPlugin.getData(this)
            .getString("widget_thermostat_catalog", null) ?: return emptyList()
        return try {
